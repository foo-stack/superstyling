...
function SizeModalDemo() {
  ...
  return (
    <HStack gap="$2" flexWrap="wrap">
      ...
      <Modal isOpen={size !== null} onClose={() => setSize(null)} size={(size as never) ?? "md"}>
        <Modal.Overlay />
        <Modal.Content>
          <Modal.Header>
  ...
  return (
    <HStack gap="$2" flexWrap="wrap">
      ...
      <Modal
        ...
        motionPreset={(motion as never) ?? "scale"}
      >
        <Modal.Overlay />
        <Modal.Content>
          <Modal.Header>
...
export default function ModalPage() {
  return (
    <DocsPage currentPath="/components/modal">
      <H1>Modal</H1>
      <StatusRow tier="1" platforms="Cross-platform" since="v0.1.0" />
      <P>
        Modal dialog with an overlay, focus trap, and body scroll lock. Cross-platform: Tamagui
        Dialog under the hood, so native gets a native-feeling overlay and web gets a portaled
        fixed-position layer.
      </P>
      <H2>Import</H2>
      <DocsCodeBlock language="tsx">{`import { Modal } from "@superstyling/core";`}</DocsCodeBlock>
      <H2>Basic modal</H2>
      <ComponentDemo
      ...
      <PropsTable
        props={[
          ...
          },
          {
            ...
            type: "() => void",
            required: true,
            description: "Called when the user requests close (overlay click, close button, ESC).",
          },
          {
            name: "size",
