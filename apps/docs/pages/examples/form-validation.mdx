...
import { Alert, Box, Button, FormControl, Heading, Input, Text, Textarea, VStack } from "@superstyling/core";
...
export function FormPreview() {
  ...
  const [touched, setTouched] = useState({ name: false, email: false, password: false, bio: false });
  const [submitted, setSubmitted] = useState(false);
  const errors = validate(values);
  const allValid = Object.keys(errors).length === 0;
  function set(k, v) { setValues((p) => ({ ...p, [k]: v })); }
  function touch(k) { setTouched((p) => ({ ...p, [k]: true })); }
  function show(k) { return touched[k] && !!errors[k]; }
  if (submitted) {
    return (
    ...
    <Box maxWidth={420}>
      <VStack gap="$4">
        ...
        <FormControl isRequired isInvalid={show("name")}>
          ...
          <Input value={values.name} onChangeText={(v) => set("name", v)} onBlur={() => touch("name")} placeholder="Ada Lovelace" />
          <FormControl.ErrorMessage>{errors.name}</FormControl.ErrorMessage>
        </FormControl>
        <FormControl isRequired isInvalid={show("email")}>
          <FormControl.Label>Email</FormControl.Label>
          <Input type="email" value={values.email} onChangeText={(v) => set("email", v)} onBlur={() => touch("email")} placeholder="you@example.com" />
          <FormControl.ErrorMessage>{errors.email}</FormControl.ErrorMessage>
        </FormControl>
        <FormControl isRequired isInvalid={show("password")}>
          <FormControl.Label>Password</FormControl.Label>
          <Input type="password" value={values.password} onChangeText={(v) => set("password", v)} onBlur={() => touch("password")} />
          <FormControl.HelperText>At least 8 characters.</FormControl.HelperText>
          <FormControl.ErrorMessage>{errors.password}</FormControl.ErrorMessage>
        </FormControl>
        <FormControl isInvalid={show("bio")}>
          <FormControl.Label>Bio</FormControl.Label>
          <Textarea value={values.bio} onChangeText={(v) => set("bio", v)} onBlur={() => touch("bio")} rows={3} placeholder="Tell us about yourself" />
